...
* `country_name` â€“ this column is useless since all of the states are from the US. We can see this if we turn it into a factor, using the `str()` function or looking at the Environment tab, but we won't do that here.
...
But wait, we get a warning message that NAs are introduced. Letâ€™s us the `summary()` function to look at the three columnsâ€”`postal_code`, `postal_code2`, and `zip`. We see that, indeed, over 300 NAs are created. What is going on? If we scroll back up to our original view of the `mooc1_states` dataframe using the tail function, we see that the reason R made the column a character type in the first place is that some zip codes include a dash and 4 extra numbers. R must not have defined the dash and extra numbers as the number format, but rather the character format. If we look at `mooc1` using `summary()` we donâ€™t see those in `zip`. In fact, all numbers are only five digits. These extra four numbers are called â€œ+four codesâ€ and are used to make zip codes even more specific. We do not need these and `mooc1` does not have them.
...
Now that we have the only five numbers in postal code 2, we can retry the `as.integer()` function. After doing that, letâ€™s run the `summary()` function and make sure things worked. Ok. That worked. Now, both `zip` and `postal_code2` are in the integer format.
...
So, which one of these methods do we want? Well, letâ€™s use the wrong one first, just to illustrate why it is important to use the correct join method. Letâ€™s use the `full_join()`, which keeps everything from both dataframes, whether the rows march up or not. First, letâ€™s check the size of `mooc1` before and then after using `dim()` This is how many rows we want to have when we are done. 
