...
When tidying your data, itâ€™s important to make sure there are very few mistakes, outliers and missing data.  Letâ€™s focus on a variable, revenue, since revenue is critical to any business and check for and deal with these issues.First, letâ€™s make sure that revenue has no missing values. We can check that with the `summary()` function. 
