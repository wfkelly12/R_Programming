...
Viewing, understanding, and exploring data is sometimes called EDAâ€”exploratory data analysis. The point of examining data, and having data in the first place, is to gain business insights from the data. Thus, after we have extracted and cleaned our data, it is time to start exploring it. 
...
There are two files that we have created for you for this videoâ€”the .rmd file with the code and the instructions and the .rds file, which has the data we will use in this lesson. First, open the .rmd file. I already have it open.

Next, letâ€™s load the packages we are going to need. We are going load the package Tidyverse. To load a package, we need to call the function `library()`. We then put the package into the parentheses of the function. If you have never downloaded tidyverse you will need to do that first. 
...
Let's pause here to define some terms that are often used to describe data. R classifies and stores data in different format”such as numeric, character, factors, etc., but today let's introduce some other terms that are commonly used outside of R to describe data formats and see what they mean. Now that the data is loaded, let's define 1) continuous variables and 2) categorical variables. 
...
Next, we add the second layer. This is the layer that tells R what graph to plot. We are going to plot a histogram so we add that function. We can add more than one plot layer if we want. These plots layers can get complicated and have their own aesthetics, but we won't get into that too much. Let's run this first graph. 
